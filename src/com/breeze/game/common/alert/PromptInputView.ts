...
public btnClose:eui.Image;
        ...
        private onTextInputChange(evt: egret.Event):void
		{
            ...
            if(t.max){
                if(num > t.max){
                    TipManagerCommon.getInstance().showLanTip("CN_190");
                    return;
                }
            }
            ...
            if(t.min){
                if(num < t.min){
                    TipManagerCommon.getInstance().showLanTip("CN_191");
                    return;
                }
            }
            ...
            if(t.total){
                if(num > t.total){
                    TipManagerCommon.getInstance().showLanTip("CN_192");
                    return;
                }
            }
        ...
        private onConfirm()
        {
            ...
            if(count <= 0){
                TipManagerCommon.getInstance().showLanTip("CN_193");
                return;
            }

            if(this.max){
                if(count > this.max){
                    TipManagerCommon.getInstance().showLanTip("CN_194",this.max);
                    return;
                }
            }
            if(this.min){
                if(count < this.min){
                    TipManagerCommon.getInstance().showLanTip("CN_195",this.max);
                    return;
                }
            }
